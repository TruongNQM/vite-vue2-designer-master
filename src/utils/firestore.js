...
export const getDocumentLimitNext = async (limitCount) => {
  const q = query(
    collection(database, import.meta.env.VITE_PROJECT_ID),
  ...
  const next = query(collection(database, import.meta.env.VITE_PROJECT_ID), startAfter(lastVisible), limit(limitCount));
  return dataReturn;
};
